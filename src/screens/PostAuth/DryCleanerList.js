...
import { StyleSheet, Text, View, KeyboardAvoidingView, TouchableOpacity, Dimensions, FlatList, Image } from 'react-native';
...
import { setDrycleanerList, setSelectedDryCleaner } from '../../state/reducers/DrycleanerReducer';
    ...
    const renderItems = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <View style={styles.circle}>
                            <Image style={styles.user} source={require('../../assets/man.png')} />
                        </View>
                        <View style={{ width: 10 }} />
                ...
                <View><Text style={styles.label}>Availibility</Text></View>
                <View style={{ height: 10 }} />
                {item.availability.map((single, index) => {
                    return (
                        <View style={{ flexDirection: 'row', marginBottom: 10,alignItems:'center' }} key={index}>
                            <Text style={styles.days}>{single.day} :- </Text>
                            <View style={{ width: 10 }} />
                            <Image style={{width:15,height:15,resizeMode:'contain',tintColor:"#F99025"}} source={require('../../assets/clock.png')} />
                            <View style={{ width: 10 }} />
                        ...
                        >
                            {stateList.map((state, index) => {
                                return (
                                    <Picker.Item style={{
                                        color: Colors.BLACK
                                    }} key={index} label={state.stateName} value={state.stateSlug} />
                                )
                            })}
